...
const DST_TOKEN_ADDRESS = "0xad6d458402f60fd3bd25163575031acdce07538d";
...
export default function ConvertEthToDai() {
    ...
    async function getEstimate() {
        ...
        // Get the ETH qty to convert from
        const ethQtyWei = web3.utils.toWei(ethQty, 'ether');
        console.log("ethQtyWei : ", ethQtyWei);
        ...
        const rate = await KYBER_NETWORK_PROXY_CONTRACT.methods.getExpectedRate(SRC_TOKEN_ADDRESS, DST_TOKEN_ADDRESS, ethQtyWei).call();
        console.log("JSON.stringify(rate): ", JSON.stringify(rate));

        // calculate dstQty
        const dstQty = calcDstQty(ethQtyWei, 18, 18, rate.expectedRate)
        console.log("dstQty: ", dstQty / 10 ** 18)
        setDaiQty(dstQty / 10 ** 18);
    }
    ...
    async function convertEthToDai() {
        ...
        // Get the ETH qty to convert from form
        const ethQtyWei = web3.utils.toWei(ethQty, 'ether');
        console.log("ethQtyWei : ", ethQtyWei);
        ...
        let tx = await KYBER_NETWORK_PROXY_CONTRACT.methods
            .trade(SRC_TOKEN_ADDRESS, ethQtyWei, DST_TOKEN_ADDRESS, userAddr, MAX_ALLOWANCE, rate.expectedRate, REF_ADDRESS)
            .send({ from: userAccount, value: ethQtyWei })
        ...
        <Layout>
            <div className="flex justify-center">
                <div className="w-1/2 flex flex-col pb-12">
                    ...
                    />
                    <button onClick={convertEthToDai} className="font-bold mt-4 bg-purple-500 text-white rounded p-4 shadow-lg">
                        Convert
                    </button>
